...
## 📥 Download and Install bilibili-cli
...
- Look for a file that ends with `.exe`. This is the program you will run.

### Step 3: Download the Program

- Click the `.exe` file to download it to your computer.  
- Choose a folder you can easily access, like **Downloads** or **Desktop**.

### Step 4: Run bilibili-cli

- Open File Explorer and go to the folder with the `.exe` file.  
- Double-click the file to start bilibili-cli.

Windows may ask if you want to allow this app to make changes to your device. Click **Yes** to continue.

### Step 5: Use bilibili-cli

The program opens in a terminal window. You can start by typing commands like:

- `search <keyword>` to look for videos  
- `user <username>` to see a user profile  
- `feed` to display your feed  
...
|-----------------|-----------------------------------------|-----------------------|
| `search <text>` | Find videos matching your search words | `search funny cats`    |
| `user <name>`   | Show information about a Bilibili user | `user bilibiliuser01`  |
| `feed`          | Display the latest videos from your feed | `feed`                |
| `play <id>`     | Open a video by ID in your web browser  | `play BV1xx411c7hL`   |
...
- Try redownloading the `.exe` file to avoid a corrupted download.  
